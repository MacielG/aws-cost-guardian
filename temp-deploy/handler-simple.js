...
module.exports.app = async (event) => {
  ...
  try {
    ...
    // GET /admin/settings - Configurações admin (requer grupo Admin)
    if (event.path === '/admin/settings' && event.httpMethod === 'GET') {
      const user = await authenticateUser(event);
      if (!user) {
        return {
          statusCode: 401,
      ...
      try {
        // Buscar configurações atuais (armazenadas em DynamoDB)
        let settings = {
          commissionRate: 0.30,
        ...
        try {
          const result = await dynamoDb.send(new GetCommand({
            ...
            Key: {
              id: 'SYSTEM',
              sk: 'SETTINGS#COMMISSION'
            }
          }));
          if (result.Item) {
            settings = {
              commissionRate: result.Item.commissionRate || 0.30,
              updatedAt: result.Item.updatedAt || new Date().toISOString(),
              updatedBy: result.Item.updatedBy || 'system'
            };
          }
        } catch (dbError) {
          console.warn('Erro ao buscar configurações:', dbError.message);
        }

        // Buscar cupons
        let coupons = [];
        try {
          const couponResult = await dynamoDb.send(new QueryCommand({
            TableName: process.env.DYNAMODB_TABLE,
            IndexName: 'CustomerDataIndex',
            KeyConditionExpression: 'id = :id AND begins_with(sk, :prefix)',
            ExpressionAttributeValues: {
          ...
          coupons = couponResult.Items || [];
        } catch (dbError) {
          console.warn('Erro ao buscar cupons:', dbError.message);
        }

        // Buscar promoções
        let promotions = [];
        try {
          const promoResult = await dynamoDb.send(new QueryCommand({
            TableName: process.env.DYNAMODB_TABLE,
            IndexName: 'CustomerDataIndex',
            KeyConditionExpression: 'id = :id AND begins_with(sk, :prefix)',
            ExpressionAttributeValues: {
          ...
          promotions = promoResult.Items || [];
        } catch (dbError) {
          console.warn('Erro ao buscar promoções:', dbError.message);
        }
        ...
        return {
          statusCode: 500,
          headers: corsHeaders,
          body: JSON.stringify({ message: 'Erro ao buscar configurações' })
        };
      }
    }

    // PUT /admin/settings - Atualizar configurações (requer grupo Admin)
    if (event.path === '/admin/settings' && event.httpMethod === 'PUT') {
      const user = await authenticateUser(event);
      if (!user) {
        return {
          statusCode: 401,
      ...
      try {
        ...
        if (typeof commissionRate !== 'number' || commissionRate < 0 || commissionRate > 1) {
          return {
            statusCode: 400,
            headers: corsHeaders,
            body: JSON.stringify({ message: 'Taxa de comissão deve ser entre 0 e 1' })
          };
        }

        // Salvar configuração no DynamoDB
        ...
        return {
          statusCode: 500,
          headers: corsHeaders,
          body: JSON.stringify({ message: 'Erro ao atualizar configurações' })
        };
      }
    }

    // POST /admin/coupons - Criar cupom (requer grupo Admin)
    if (event.path === '/admin/coupons' && event.httpMethod === 'POST') {
      const user = await authenticateUser(event);
      if (!user) {
        return {
          statusCode: 401,
      ...
      try {
        const body = JSON.parse(event.body || '{}');
        const { code, discountType, discountValue, validUntil, maxUses, description } = body;

        if (!code || !discountType || typeof discountValue !== 'number') {
          return {
            statusCode: 400,
            headers: corsHeaders,
            body: JSON.stringify({ message: 'Código, tipo de desconto e valor são obrigatórios' })
          };
        }
        ...
        try {
          ...
          if (existing.Item) {
            return {
              statusCode: 400,
              headers: corsHeaders,
              body: JSON.stringify({ message: 'Cupom já existe' })
            };
          }
        } catch (dbError) {
        ...
        return {
          statusCode: 200,
          headers: corsHeaders,
          body: JSON.stringify({ message: 'Cupom criado com sucesso' })
        };
      } catch (error) {
        ...
        return {
          statusCode: 500,
          headers: corsHeaders,
          body: JSON.stringify({ message: 'Erro ao criar cupom' })
        };
      }
    }

    // DELETE /admin/coupons/{code} - Excluir cupom (requer grupo Admin)
    if (event.path.startsWith('/admin/coupons/') && event.httpMethod === 'DELETE') {
      const user = await authenticateUser(event);
      if (!user) {
        return {
          statusCode: 401,
      ...
      try {
        ...
        // Verificar se cupom existe e pode ser excluído
        const existing = await dynamoDb.send(new GetCommand({
          TableName: process.env.DYNAMODB_TABLE,
          Key: {
            id: 'SYSTEM',
        ...
        if (!existing.Item) {
          return {
            statusCode: 404,
            headers: corsHeaders,
            body: JSON.stringify({ message: 'Cupom não encontrado' })
          };
        }

        if (existing.Item.usedCount > 0) {
          return {
            statusCode: 400,
            headers: corsHeaders,
            body: JSON.stringify({ message: 'Não é possível excluir cupom já utilizado' })
          };
        }
        ...
        return {
          statusCode: 200,
          headers: corsHeaders,
          body: JSON.stringify({ message: 'Cupom excluído com sucesso' })
        };
      } catch (error) {
        ...
        return {
          statusCode: 500,
          headers: corsHeaders,
          body: JSON.stringify({ message: 'Erro ao excluir cupom' })
        };
      }
    }
    ...
    // DELETE /admin/promotions/{id} - Excluir promoção (requer grupo Admin)
    if (event.path.startsWith('/admin/promotions/') && event.httpMethod === 'DELETE') {
      const user = await authenticateUser(event);
      if (!user) {
        return {
          statusCode: 401,
      ...
      try {
        ...
        if (!existing.Item) {
          return {
            statusCode: 404,
            headers: corsHeaders,
            body: JSON.stringify({ message: 'Promoção não encontrada' })
          };
        }
        ...
        return {
          statusCode: 200,
          headers: corsHeaders,
          body: JSON.stringify({ message: 'Promoção excluída com sucesso' })
        };
      } catch (error) {
        ...
        return {
          statusCode: 500,
          headers: corsHeaders,
          body: JSON.stringify({ message: 'Erro ao excluir promoção' })
        };
      }
    }

    // GET /admin/claims - Listar reivindicações (requer grupo Admin)
    if (event.path === '/admin/claims' && event.httpMethod === 'GET') {
      const user = await authenticateUser(event);
      if (!user) {
        return {
          statusCode: 401,
      ...
      try {
        // Buscar todas as reivindicações SLA
        const claims = [];
        try {
          ...
          claims.push(...(result.Items || []));
        } catch (dbError) {
          console.warn('Erro ao buscar claims:', dbError.message);
        }
        ...
        console.error('Erro ao buscar claims admin:', error);
        return {
          statusCode: 500,
          headers: corsHeaders,
          body: JSON.stringify({ message: 'Erro ao buscar claims' })
        };
      }
    }

    // PUT /admin/claims/{customerId}/{claimId}/status - Atualizar status da reivindicação
    if (event.path.match(/^\/admin\/claims\/[^\/]+\/[^\/]+\/status$/) && event.httpMethod === 'PUT') {
      const user = await authenticateUser(event);
      if (!user) {
        return {
          statusCode: 401,
      ...
      try {
        ...
        return {
          statusCode: 500,
          headers: corsHeaders,
          body: JSON.stringify({ message: 'Erro ao atualizar status' })
        };
      }
    }

    // POST /admin/claims/{customerId}/{claimId}/create-invoice - Criar fatura para reivindicação
    if (event.path.match(/^\/admin\/claims\/[^\/]+\/[^\/]+\/create-invoice$/) && event.httpMethod === 'POST') {
      const user = await authenticateUser(event);
      if (!user) {
        return {
          statusCode: 401,
      ...
      try {
        ...
        const claim = await dynamoDb.send(new GetCommand({
          TableName: process.env.DYNAMODB_TABLE,
          Key: {
            id: customerId,
            sk: `CLAIM#${claimId}`
        ...
        if (!claim.Item) {
          return {
            statusCode: 404,
            headers: corsHeaders,
            body: JSON.stringify({ message: 'Reivindicação não encontrada' })
          };
        }

        // Criar fatura via Stripe (simulado)
        const invoiceId = `inv_${Date.now()}`;

        // Atualizar reivindicação para REFUNDED
        await dynamoDb.send(new UpdateCommand({
          TableName: process.env.DYNAMODB_TABLE,
          Key: {
            id: customerId,
          ...
          UpdateExpression: 'SET #status = :status, stripeInvoiceId = :invoiceId, updatedAt = :updatedAt',
          ExpressionAttributeNames: {
            '#status': 'status'
          },
          ExpressionAttributeValues: {
        ...
        return {
          statusCode: 500,
          headers: corsHeaders,
          body: JSON.stringify({ message: 'Erro ao criar fatura' })
        };
      }
    }
